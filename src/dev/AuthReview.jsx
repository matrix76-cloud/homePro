      ...
      {guideOpen && (
        <div style={{ position: 'fixed', top: 64, right: 16, width: 460, maxHeight: 'calc(100vh - 90px)', overflowY: 'auto', background: '#fff', border: `1px solid ${C.ink}`, zIndex: 60, padding: '16px 18px 20px', fontSize: 15, lineHeight: 1.6, color: C.ink, boxShadow: '0 8px 24px rgba(0,0,0,0.12)' }}>
          ...
          <ol style={{ margin: '6px 0 12px', paddingLeft: 20 }}>
            <li><b>캐시백 직거래</b> — a1이 오더 접수(캐시백 정액) → b1이 수락 → b1 오더 상세에 a1 계좌·금액과 [토스로 송금]·[계좌 복사]·[입금 완료] → a1에게 푸시 → a1 [입금 확인] → 그제야 b1 체크인 열림. 플랫폼은 돈을 안 만짐.</li>
            <li><b>보험 필수</b> — 보험 없는 b1이 [수락하기]를 누르면 "보험 가입 후 수락"으로 막힘. 확인=이 오더만 건당 결제(토스 테스트 결제창), 취소=안심케어 탭에서 월·1년 가입. 가입자는 자동 적용.</li>
            ...
            <li><b>월 구독</b> — 마이페이지 구독 관리(또는 메인 상단 차수 표시) → "H-포인트 사용하기" 체크 → 전액/직접 입력 → 나머지만 토스 결제 → 1차수(0초 수락). 미구독은 5분 뒤 수락.</li>
